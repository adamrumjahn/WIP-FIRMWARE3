...
void setMotorCurrentByVoltage(uint8_t channel, float level) {
	uint16_t raw_level = 4095*(level/2.048);
	if (raw_level > 4095) raw_level = 4095;
...
void Commands::processMCode(GCode *com) {
    ...
    switch( com->M ) {
        ...
        case 104: // M104 temperature
            ...
            break;
		case 122:
			#if (HAVE_TMC2130DRIVER) && (X_IS_TMC2130)
				HAL::SelectSPIChip(SPI_X);
				res = Printer::stepperX.read_STAT();
				Com::printFLN(PSTR("X-AXIS: "), res);
				Com::printFLN(PSTR((Printer::stepperX.isReset() ? "RESET: Yes" : "RESET: No ")));
				Com::printFLN(PSTR((Printer::stepperX.isError() ? "ERROR: Yes " : "ERROR: No ")));
				Com::printFLN(PSTR((Printer::stepperX.isStallguard() ? "SLGRD: Yes " : "SLGRD: No ")));
				Com::printFLN(PSTR((Printer::stepperX.isStandstill() ? "STILL: Yes " : "STILL: No ")));
				Com::printFLN(PSTR("-----"));
				HAL::SelectSPIChip(SPI_NONE);
			#endif
			//
			#if (HAVE_TMC2130DRIVER) && (Y_IS_TMC2130)
				HAL::SelectSPIChip(SPI_Y);
				res = Printer::stepperY.read_STAT();
				Com::printFLN(PSTR("Y-AXIS: "), res);
				Com::printFLN(PSTR((Printer::stepperY.isReset() ? "RESET: Yes" : "RESET: No ")));
				Com::printFLN(PSTR((Printer::stepperY.isError() ? "ERROR: Yes " : "ERROR: No ")));
				Com::printFLN(PSTR((Printer::stepperY.isStallguard() ? "SLGRD: Yes " : "SLGRD: No ")));
				Com::printFLN(PSTR((Printer::stepperY.isStandstill() ? "STILL: Yes " : "STILL: No ")));
				Com::printFLN(PSTR("-----"));
				HAL::SelectSPIChip(SPI_NONE);
			#endif
				//
			#if (HAVE_TMC2130DRIVER) && (Z_IS_TMC2130)
				HAL::SelectSPIChip(SPI_Z);
				res = Printer::stepperZ.read_STAT();
				Com::printFLN(PSTR("Z-AXIS: "), res);
				Com::printFLN(PSTR((Printer::stepperZ.isReset() ? "RESET: Yes" : "RESET: No ")));
				Com::printFLN(PSTR((Printer::stepperZ.isError() ? "ERROR: Yes " : "ERROR: No ")));
				Com::printFLN(PSTR((Printer::stepperZ.isStallguard() ? "SLGRD: Yes " : "SLGRD: No ")));
				Com::printFLN(PSTR((Printer::stepperZ.isStandstill() ? "STILL: Yes " : "STILL: No ")));
				Com::printFLN(PSTR("-----"));
				HAL::SelectSPIChip(SPI_NONE); 
				Printer::getSGValue();
			#endif
				//
			#if (HAVE_TMC2130DRIVER) && (E0_IS_TMC2130)
				HAL::SelectSPIChip(SPI_E0);
				res = Printer::stepperE0.read_STAT();
				Com::printFLN(PSTR("E0-AXIS: "), res);
				Com::printFLN(PSTR((Printer::stepperE0.isReset() ? "RESET: Yes" : "RESET: No ")));
				Com::printFLN(PSTR((Printer::stepperE0.isError() ? "ERROR: Yes " : "ERROR: No ")));
				Com::printFLN(PSTR((Printer::stepperE0.isStallguard() ? "SLGRD: Yes " : "SLGRD: No ")));
				Com::printFLN(PSTR((Printer::stepperE0.isStandstill() ? "STILL: Yes " : "STILL: No ")));
				Com::printFLN(PSTR("-----"));
				HAL::SelectSPIChip(SPI_NONE);
			#endif
			break;
...
#if (Z_IS_TMC2130)
		case 902:
			// M902 Set Coolstep settings for Z
			if (com->hasA()) {
				Printer::setZsemin((uint32_t)com->A);
			}
			if (com->hasB()) {
				Printer::setZsemax((uint32_t)com->B);
			}
			if (com->hasC()) {
				Printer::setZsedn((uint32_t)com->C);
			}
			if (com->hasD()) {
				Printer::setZseup((uint32_t)com->D);
			}
			break;
#endif
#if (Z_IS_TMC2130)
		case 903:
			// M903 Set Z TCOOLTHRS
			if (com->hasZ()) {
				Printer::setZTCOOLTHRS((uint32_t)com->Z);
			}
			break;
#endif
#if (Z_IS_TMC2130)
		case 904:
			// M904 Set Z current when zprobing Z=ZProbe current Y=Normal Z current
			if (com->hasY()) {
				Printer::normalZCurrent = com->Y;
				setMotorCurrentByVoltage(2, (float)com->Y);
				Com::printFLN(PSTR("Z normal current set to:"), com->Y);
			}
			if (com->hasZ()) {
				Printer::zProbeCurrentSetting = com->Z;
				Com::printFLN(PSTR("Z Probe current set to:"), com->Z);
			}
			break;
#endif
#if (Z_IS_TMC2130)
		case 905:
			// M905 Set Z axis Stallguard Threshold
			//#if STEPPER_CURRENT_CONTROL != CURRENT_CONTROL_MANUAL
			// If "S" is specified, use that as initial default value, then update each axis w/ specific values as found later.
			if (com->hasZ()) Printer::setZSGThreshold((int)com->Z);
			//#endif			
			break;
#endif
		case 906:
			// M906 Set digital trimpot/DAC motor current using axis codes. Values are interpreted directly as the voltage to output, between 0 and 2.048 
			// (assuming we force the gain to x1 with an internal reference).
			#if STEPPER_CURRENT_CONTROL != CURRENT_CONTROL_MANUAL
				// If "S" is specified, use that as initial default value, then update each axis w/ specific values as found later.
				if (com->hasS()) {
					for (int i = 0; i < 10; i++) {
						setMotorCurrentByVoltage(i, com->S);
					}
				}

				if (com->hasX()) setMotorCurrentByVoltage(0, (float)com->X);
				if (com->hasY()) setMotorCurrentByVoltage(1, (float)com->Y);
				if (com->hasZ()) setMotorCurrentByVoltage(2, (float)com->Z);
				if (com->hasE()) setMotorCurrentByVoltage(3, (float)com->E);
			#endif			
			break;
        case 907: 
			// M907 Set digital trimpot/DAC motor current using axis codes. Values are interpreted as a percent of the max, as set by MCP4728_VOUT_MAX, the reference, and the gain
			#if STEPPER_CURRENT_CONTROL != CURRENT_CONTROL_MANUAL
				// If "S" is specified, use that as initial default value, then update each axis w/ specific values as found later.
				if(com->hasS()) {
					for(int i = 0; i < 10; i++) {
