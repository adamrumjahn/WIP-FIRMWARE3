...
public:
	...
	static void setZSGThreshold(uint8_t threshold); 
	static void setZTCOOLTHRS(uint32_t threshold);
	static int getSGValue();
	static float zProbeCurrentSetting;
	static float normalZCurrent;
	static void setZsemin(uint32_t setZsemin);
	static void setZsemax(uint32_t setZsemax);
	static void setZsedn(uint32_t setZsedn);
	static void setZseup(uint32_t setZseup);
#endif
#if (E0_IS_TMC2130)
