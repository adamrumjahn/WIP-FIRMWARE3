...
public:
    ...
    inline void checkEndstops()
    {
        if(isCheckEndstops())
        {
           ...
           else if(isZPositiveMove() && Endstops::zMax() && !Printer::isZProbingActive())	// Added the check to make sure we aren't z probing, for the case that z-probe pin is same as z-max
           {
	           #if MAX_HARDWARE_ENDSTOP_Z
	           Printer::stepsRemainingAtZHit = stepsRemaining;
	           #endif
