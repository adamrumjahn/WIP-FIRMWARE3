...
float Printer::runZMaxProbe() {
	...
	GCode gc = GCode();
	String gcs = "M906 z" + String(Printer::zProbeCurrentSetting);
	char gcca[20];
	gcs.toCharArray(gcca, 20, 0);
	gc.parseAscii(gcca, false);
	Commands::processMCode(&gc);
	PrintLine::moveRelativeDistanceInSteps(0, 0, probeDepth, 0, EEPROM::zProbeSpeed(), true, true);
	gcs = "M906 z" + String(Printer::normalZCurrent);
	gcs.toCharArray(gcca, 20, 0);
	gc.parseAscii(gcca, false);
	Commands::processMCode(&gc);
...
float Printer::runZProbe(bool first,bool last,uint8_t repeat,bool runStartScript) {
	...
	HAL::delayMilliseconds(2000); // adding delay here for TMC2130 drivers
    ...
    for(int8_t r = 0; r < repeat; r++) {
        ...
        if(r + 1 < repeat) {
			...
			setZProbingActive(true);	// added to prevent zmax from stopping the move
