...
void Endstops::update() {
   ...
   // DEBUG_MSG2_FAST("test x min",(int)READ(X_MIN_PIN));
    ...
    lastRead2 = newRead2;
#endif
	// Debugging Stallguard for z-probe
...
#if (X_IS_TMC2130)
Trinamic_TMC2130 Printer::stepperX(SPI_CS);
#endif
#if (Y_IS_TMC2130)
Trinamic_TMC2130 Printer::stepperY(SPI_CS);
#endif
#if (Z_IS_TMC2130)
Trinamic_TMC2130 Printer::stepperZ(SPI_CS);
#endif
#if (E0_IS_TMC2130)
Trinamic_TMC2130 Printer::stepperE0(SPI_CS);
#endif
#if (E1_IS_TMC2130)
Trinamic_TMC2130 Printer::stepperE1(SPI_CS);
#endif
#if (E2_IS_TMC2130)
Trinamic_TMC2130 Printer::stepperE2(SPI_CS);
#endif
	...
	#if (Z_IS_TMC2130)
	void Printer::setZSGThreshold(uint8_t threshold) {
			HAL::SelectSPIChip(SPI_Z);
			stepperZ.set_sgt(threshold);
			Com::printFLN(PSTR("Z Stallguard set to "), threshold);
			HAL::SelectSPIChip(SPI_NONE);
		}
	void Printer::setZTCOOLTHRS(uint32_t threshold) {
		HAL::SelectSPIChip(SPI_Z);
		stepperZ.set_TCOOLTHRS(threshold);
		Com::printFLN(PSTR("Z COOLTHRS set to "), threshold);
		HAL::SelectSPIChip(SPI_NONE);
	}

  float Printer::zProbeCurrentSetting = 1.2; // TMC was 1.2
  float Printer::normalZCurrent = 0.6;  // TMC was 1.2

	int Printer::getSGValue() {
		HAL::SelectSPIChip(SPI_Z);
		uint32_t val = 0;
		stepperZ.read_REG(TMC_REG_DRV_STATUS, &val);
		// select just the SG Result from the status reg
		val = val & 0b1111111111;
		Com::printFLN(PSTR("Z SG Value: "), val);
		HAL::SelectSPIChip(SPI_NONE);		
		return val;
	}
	void Printer::setZsemin(uint32_t setZsemin) {
		HAL::SelectSPIChip(SPI_Z);
		stepperZ.set_semin(setZsemin);
		Com::printFLN(PSTR("Set Z semin: "), setZsemin);
		HAL::SelectSPIChip(SPI_NONE);
	}
	void Printer::setZsemax(uint32_t setZsemax) {
		HAL::SelectSPIChip(SPI_Z);
		stepperZ.set_semax(setZsemax);
		Com::printFLN(PSTR("Set Z semax: "), setZsemax);
		HAL::SelectSPIChip(SPI_NONE);
	}
	void Printer::setZsedn(uint32_t setZsedn) {
		HAL::SelectSPIChip(SPI_Z);
		stepperZ.set_sedn(setZsedn);
		Com::printFLN(PSTR("Set Z sedn: "), setZsedn);
		HAL::SelectSPIChip(SPI_NONE);
	}
	void Printer::setZseup(uint32_t setZseup) {
		HAL::SelectSPIChip(SPI_Z);
		stepperZ.set_seup(setZseup);
		Com::printFLN(PSTR("Set Z seup: "), setZseup);
		HAL::SelectSPIChip(SPI_NONE);
	}
	#endif
#endif
